...
const setLink = (id, value, isEmail = false) => {
  ...
  el.innerHTML = `<a href="${escapeHtml(href)}" class="contact_link" ${!isEmail ? "target='_blank' rel='noopener noreferrer'" : ''}>${escapeHtml(value)}</a>`
}
...
const createSeeMoreCard = () => `
  <a href='../projects/' class='project_card_link'>
    <article class='project_card'>
...
const createWorkCompanyCard = (organization, roles) => {
  ...
  const overallEnd = hasCurrentRole
    ? 'Present'
    : [...sortedRoles].map((role) => role.end_date || '').filter(Boolean).sort().slice(-1)[0] || ''
